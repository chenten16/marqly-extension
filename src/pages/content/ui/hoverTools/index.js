import { HIGHLIGHT_CLASS, updateColor as updateHighlightColor, remove as removeHighlight } from '../highlight/index.js';
...
function initializeHoverTools() {
  $.get(chrome.runtime.getURL('assets/templates/tooltip.html'), data => {
    hoverToolEl = $(data);
    hoverToolEl.hide();
    ...
    const colorsBtns = hoverToolEl.find('.highlighter--icon-change-color');
    let deleteIcon = hoverToolEl.find('.delete-icon')[0];
    deleteIcon.setAttribute('src', chrome.runtime.getURL('assets/img/trash.svg'));
    ...
    colorsBtns.each(function (_, el) {
      const colorId = $(this).data('colorId');
      $(this).on('click', function () {
        onChangeColorBtnClicked(colorId);
      });
    });
  });
...
function onChangeColorBtnClicked(colorId) {
  const highlightId = currentHighlightEl.getAttribute('data-highlight-id');
